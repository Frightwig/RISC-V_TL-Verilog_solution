...
\TLV
   ...
   |cpu
      ...
      @1
         ...
         //根据操作码及funct3，产生全部基础指令（除FENCE, ECALL, EBREAK）。
      ...
      @3
         ...
         //有效信号，每三个周期内只能有一个跳转或加载指令发生，避免依赖。
         $valid = !(>>1$valid_load || >>2$valid_load || >>1$valid_taken_br || >>2$valid_taken_br);
         
         //Register File Write。写寄存器堆将运算结果放入。   
         $rf_wr_en = $rd_valid && ($rd != 5'b0) && $valid;    //0号寄存器始终为0。且只有load或分支信号执行完成后，才可重新写入数据至寄存器堆
   ...
   *failed = 1'b0;
   ...
   m4+cpu_viz(@5)    // 提供的可视化,@4延迟至4周期再输出
\SV
   endmodule
