...
\TLV
   //3-cycle-RISC-V & valid
   //在之前的单周期核的基础上，将其改为3周期CPU：译码、读取、执行。
   //但直接连会出现两个主要问题：（1）：RF-WR还没写入数据，下一次周期已需要用到RF-RD （2）还没确定是否需要分支指令，PC就已经指向下一指令
   //解决方法，使用VALID信号来控制PC。每三周期有效一次，防止出现进入分支又出现别的分支情况，
   //每三周期更新一次地址，使得依赖项对齐
   ...
   |cpu
      @0
         ...
         $pc[31:0] = >>1$reset ? 32'd0 :
                     >>3$valid_taken_br ? >>3$br_tgt_pc :            //分割成多周期，读取三周期后的分支信号
                                    >>3$pc + 32'd4;                  //若没有分支信号，三周期后加4.
                                    
         $start = (>>1$reset && $reset == 0) ? 1'b1 : 1'b0;          //每一次复位之后产生start信号
         $valid = $reset ? 1'b0 :                                    //每产生start信号，周期有效。不然每三个周期有效一次
                  $start ? 1'b1 : >>3$valid;
      ...
      @2
         ...
         $src1_value[31:0] = $rf_rd_data1;   //第一源寄存器输出至alu数据
         $src2_value[31:0] = $rf_rd_data2;   //第二源寄存器输出至alu数据
      ...
      @3
         ...
         $valid_taken_br = $valid && $taken_br;       //达到产生分支跳转条件，且当前为有效周期时，才发生分支跳转。
